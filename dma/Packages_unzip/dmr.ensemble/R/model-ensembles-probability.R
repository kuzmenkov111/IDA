...
predict.ensemble.prob <- function(models, data, predf=predict, prob=FALSE, labels=NULL)
{
  ...
  else
  {
    if (is.null(labels))
      labels <- colnames(cp)
    label.all(apply(cp, 1, which.max), levels=1:2, labels=labels)
  }
}
...
hv.pred.nb.sa.p <- predict.ensemble.prob(hv.bm.nb.sa, hv.test,
                                         predf=function(...) predict(..., type="r"))
hv.pred.tree.ps.p <- predict.ensemble.prob(hv.bm.tree.ps, hv.test)
...
hv.err.p <- c(tree = hv.err.tree,
              ...
              tree.ps = err(hv.pred.tree.ps.p, hv.test$Class),
              nb = hv.err.nb,
...
lines(c(0, 10), rep(hv.err.p[1], 2), col="red")
lines(c(0, 10), rep(hv.err.p[6], 2), col="orange")
